...
mbi_2_bp = Blueprint('mbi_2', __name__, template_folder='templates')
...
def get_db_connection():
    ...
    try:
        conn = mysql.connector.connect(**DB_CONFIG_MBI_2)
        return conn
...
@mbi_2_bp.route('/api/scrap_causes')
def scrap_causes():
...
@mbi_2_bp.route('/api/production_timeline')
...
@mbi_2_bp.route('/api/user_production')
def user_production():
...
@mbi_2_bp.route('/api/histogram_data')
def histogram_data():
...
@mbi_2_bp.route('/api/latest_records')
def latest_records():
